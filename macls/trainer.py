    ...
    def train(self,
        ...
        for epoch_id in range(last_epoch, self.configs.train_conf.max_epoch):
            ...
            if local_rank == 0:
                ...
                plt.plot(self.eval_losses, label='val loss', color='coral')
                if len(self.train_losses) < 25:
                    num_train = 5
                else:
                    num_train = 15
                if len(self.eval_losses) < 25:
                    num_eval = 5
                else:
                    num_eval = 15
                smooth_train_loss = savgol_filter(self.train_losses, num_train, 3, mode='constant')
                smooth_eval_loss = savgol_filter(self.eval_losses, num_eval, 3, mode='constant')
                plt.plot(smooth_train_loss,  label='smooth train loss', color='green', linestyle='--')
                plt.plot(smooth_eval_loss,  label='smooth val loss', color='#8B4513', linestyle='--')
                plt.title('Loss curve')
                plt.xlabel('Epochs')
                plt.ylabel('Loss')
                plt.legend(loc="upper right")
                plt.savefig(os.path.join(save_model_path, 'loss.png'))
                # plt.show()
                plt.close('all')
               # 保存模型
                self.__save_checkpoint(save_model_path=save_model_path, epoch_id=epoch_id, best_acc=self.eval_acc)
